...
""" Run Utterance Order Prediction on (Bert, RoBERTa)."""
...
from src.transformers import UOPProcessor
from src.transformers import (
    WEIGHTS_NAME,
    AdamW,
    BertConfig,
    BertForUtteranceOrderPrediction,
    BertTokenizer,
    RobertaConfig,
    RobertaForUtteranceOrderPrediction,
    RobertaTokenizer,
    get_constant_schedule
)
from src.transformers import glue_compute_metrics as compute_metrics
from src.transformers import uop_convert_example_to_features as convert_examples_to_features
...
MODEL_CLASSES = {
    "bert": (BertConfig, BertForUtteranceOrderPrediction, BertTokenizer),
    "roberta": (RobertaConfig, RobertaForUtteranceOrderPrediction, RobertaTokenizer),
}
...
def train(args, train_datasets, model, tokenizer, train_len):
    ...
    for train_dataset in train_datasets:
        ...
        if int(train_dataset.tensors[0][0].size()[0]) <= 30:
            batch_size = 4
        elif int(train_dataset.tensors[0][0].size()[0]) <= 60:
            batch_size = 2
        elif int(train_dataset.tensors[0][0].size()[0]) <= 120:
            batch_size = 1
        else:
            continue
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_datasets, dev_len = load_and_cache_examples(args, tokenizer, evaluate=True)
        ...
        for eval_dataset in eval_datasets:
            if int(eval_dataset.tensors[0][0].size()[0]) > 120:
                continue
        ...
        preds = None
        out_label_ids = None
        for eval_dataloader in eval_dataloader_list:
            for batch in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                nb_eval_steps += 1
                if preds is None:
                    preds = np.expand_dims(logits.detach().cpu().numpy(), axis=0)
                    out_label_ids = inputs["labels"].detach().cpu().numpy()
                else:
                    preds = np.append(preds, np.expand_dims(logits.detach().cpu().numpy(), axis=0), axis=0)
